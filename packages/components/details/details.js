...
export default () => {
  ...
  */
  const initDetails = (element, index) => {
    ...
    element.setAttribute('moduk-polyfilled', 'true');

    // Give details element an ID if it doesn't already have one
    if (!element.id) element.setAttribute('id', `moduk-details${index}`);

    // Set content element and give it an ID if it doesn't already have one
    const content = document.querySelector(`#${element.id} .moduk-details__text`);
    if (!content.id) content.setAttribute('id', `moduk-details__text${index}`);

    // Set summary element
    const summary = document.querySelector(`#${element.id} .moduk-details__summary`);
  ...
  if (allDetails.length) {
    allDetails.forEach((element, index) => {
      if (!element.hasAttribute('moduk-polyfilled')) initDetails(element, index);
    });
  }
};
