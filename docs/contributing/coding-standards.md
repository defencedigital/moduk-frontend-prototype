...
MOD.UK frontend uses the Block Element Modifier (BEM) methodology when naming
...
This enables the MOD.UK styles to be used inside other applications, where, for example, an ID is being used to isolate a section of a page to style separately from the rest of an application; e.g. 
```scss
div#moduk-ers {
...
...
We have chosen as Nunjucks as the templating language for MOD.UK frontend components. We expose those templates as reusable chunks of code: macros. Developers import macros into their application, call them as per documentation and provide data to its arguments.
...
You can find MOD.UK frontend components in `packages/components`.
