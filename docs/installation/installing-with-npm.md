...
To use MOD.UK frontend in your projects with npm you must:
...
Install the MOD.UK frontend package into your project:

```
npm install moduk-frontend --save
```

## Configuration

You will need to import a couple of things into your project before you can start using MOD.UK frontend:
...
@import 'node_modules/moduk-frontend/packages/components/action-link/action-link';
```
...
You should include MOD.UK frontend JavaScript in your project to ensure that all users can use it successfully.
...
Include the `node_modules/moduk-frontend/dist/moduk.min.js` script in the `<head>` of your page using the `defer` attribute. 
...
```html
    <script src="path-to-assets/moduk.min.js" defer></script>
  </head>
```

```html
    <script src="node_modules/moduk-frontend/dist/moduk.min.js" defer></script>
  </head>
```
...
If you want to import assets such as the NHS logo, favicons and SVG icons, you might wish to copy the files into your project folders from the `node_modules/moduk-frontend/assets/` directory or you can reference them straight from the `node_modules` folder.
